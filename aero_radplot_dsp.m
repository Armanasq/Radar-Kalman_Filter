...
radarFigure = findall(0,'type','figure','tag','aero_radplot_dsp');
if isempty(radarFigure)
    pos = [10 40 900 650];
    radarFigure = figure('Position',pos,'Tag','aero_radplot_dsp');
end
radarFigure.HandleVisibility = 'on';
figure(radarFigure);
...
% Change HandleVisibility of the figure to 'callback' so that the figure is
% not reused for other graphics function calls.
radarFigure.HandleVisibility = 'callback';
